...
const universities = [];
graduates.forEach((graduate) => { universities.push(graduate.university); universities.sort(); });
...
const lowCaseAnimalNames = zooAnimals.map(animal => {
  let container = animal.animal_name.toLowerCase();
  return container;
});
...
*/
const lowPopulationAnimals = zooAnimals.filter((animal) => { let popSize = animal.population; return popSize < 5; });
...
*/
const populationTotal = zooAnimals.reduce((acc, currVal) => { let val = currVal.population; return acc + val; }, 0);
